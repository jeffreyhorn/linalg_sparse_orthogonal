...
| 2 | Right preconditioning for GMRES | Add right preconditioning option to `sparse_solve_gmres()`: solve A*M^{-1}*(M*x) = b instead of M^{-1}*A*x = b. The key advantage: the GMRES residual norm equals the true residual norm (no preconditioned/true residual gap). Add `precond_side` option (left/right) to `sparse_gmres_opts_t`. | 12 hrs |
