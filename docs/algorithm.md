...
### Nested Dissection (ND) — multilevel vertex separator (Sprint 22 + Sprint 23 + Sprint 24 + Sprint 25 + Sprint 26 closures)
...
5. **Recursive ordering** — on each interior partition: recurse; for subgraphs with `n ≤ ND_BASE_THRESHOLD` (default 96 — **Sprint 26 Day 5 default-flip from Sprint 22's 32**), call `sparse_reorder_amd_qg` on the leaf and splice the per-leaf permutation into the global perm[] via `vertex_id_map` (Sprint 23 Day 7).  Append the separator vertices last.  Sprint 26 Day 5's threshold flip from 32 → 96 was driven by Day 4's per-recursion-depth profile finding (88% of ND wall on Pres_Poisson concentrates at depths 6-9 = ~169 small-subgraph multilevel-pipeline calls with 60-200 ms per-call constant overhead floor).  The new threshold cuts off the recursion earlier, replacing those calls with sub-millisecond leaf-AMD invocations.  Result on Pres_Poisson: ND wall 38.1 s → 12.2 s (-67.9%) with nnz_L bit-stable (-0.21pp).  Per-fixture wins on every fixture in [-1.3, -16.4] pp range; corpus-wide wall improvement of -38% to -81% (-25pp on nos4, -10.6pp on Kuu).  See `docs/planning/EPIC_2/SPRINT_26/nd_base_threshold_decision.md`.
...
- Per-fixture advisory env-var settings (each documented in the cited decision docs).  Sprint 25 Day 9's 16-setting × 6-fixture sweep (`bench_day9_combinations.{csv,txt}`) is the cross-corpus reference for combined effects:
    - **Pres_Poisson (headline)** — `SPARSE_ND_COARSENING=hcc SPARSE_ND_COARSEN_FLOOR_RATIO=200` (Day 9 setting 13) drops ND/AMD 0.952× → **0.922×** (-3.0pp; the Sprint 25 best on this fixture).  HCC alone closes -1.5pp; ratio=200 alone closes -1.0pp; the combination composes constructively at -3.0pp.  Other combinations involving spectral or intermediate-FM-3 wash this back out.  See `docs/planning/EPIC_2/SPRINT_25/headline_summary.md`.
    - **Kuu (irregular SPD; the corpus-wide best)** — full setting `SPARSE_ND_COARSENING=hcc SPARSE_ND_COARSEN_FLOOR_RATIO=200 SPARSE_FM_INTERMEDIATE_PASSES=3 SPARSE_ND_COARSEST_BISECTION=spectral SPARSE_ND_SEP_LIFT_STRATEGY=balanced_boundary` (Day 9 setting 15) drops Kuu ND/AMD 2.275× → **1.309×** (-97pp — the largest single corpus win Sprint 25 produced).  Same setting also wins on nos4 (-26pp) and bcsstk14 (-9pp), at the cost of a marginal Pres_Poisson nnz_L drift (+0.5pp vs default) but a ~23× ND wall-time speedup (~37 s → ~1.6 s) on Pres_Poisson via spectral cuts converging close to the FM optimum.  Recommended for callers prioritising aggregate fill quality + ND wall-time over the Pres_Poisson nnz_L headline.
    - **Kuu (single-axis advisory)** — `SPARSE_FM_INTERMEDIATE_PASSES=3` alone drops Kuu ND/AMD 2.275× → **2.043×** (-23.2pp; +3.7% wall cost).  Useful for callers who want the largest Sprint-25 single-fixture win without the full setting-15 env-var stack; setting 15 still wins more (1.309×) but at a much higher cost surface.  See `intermediate_fm_decision.md`.
    - **bcsstk14 (single-axis advisory)** — `SPARSE_FM_INTERMEDIATE_PASSES=2` drops bcsstk14 ND/AMD 1.129× → 1.107× (-2.2pp, +4.7% wall) or `passes=3` 1.106× (-2.3pp, -11.4% wall — wall actually *drops* at passes=3 because the finer-level FM converges faster from a better intermediate-level starting cut).  See `intermediate_fm_decision.md`.
    - **Kuu / bcsstk14 / nos4 (Sprint 24 advisory)** — `SPARSE_ND_SEP_LIFT_STRATEGY=balanced_boundary` drops ND/AMD by 8-38 percentage points stand-alone; Kuu sees a 38pp drop (924 385 → 574 861 nnz_L).  Sprint 25's setting 15 above subsumes this with additional knobs.
    - **s3rmt3m3** — no env-var advisory; default settings are best.  Day 5's `SPARSE_FM_INTERMEDIATE_PASSES=2` sweep *regresses* this fixture's nnz_L by +1.7pp (1.009× → 1.026×) despite a ~12% wall-time drop, so it is not advisory; see `intermediate_fm_decision.md`.
    - **bcsstk14 default-flip blocker** — HCC matching produces a degenerate `sep = 0` empty separator when applied to bcsstk14, blocking the production-default flip independent of nnz_L deltas.  Sprint 26 inherits the root-cause investigation (likely a structural property of bcsstk14 interacting with HCC's `min(deg(u), deg(v))` weighting).  See `coarsening_decision.md` "Two test failures surfaced under the new defaults".
- The Pres_Poisson default-path 0.952× ratio is the cumulative effect of Sprint 23 Days 7 (leaf-AMD splice), 9-10 (gain-bucket FM lifting per-pass cost from O(n²) to O(|E|)), and 11 (multi-pass FM at the finest level).  PLAN.md's literal ≤ 0.7× (Sprint 23), ≤ 0.85× (Sprint 24), and ≤ 0.85× (Sprint 25) targets are all unmet (default 0.952×; Sprint 24 best opt-in 0.942×; Sprint 25 best opt-in **0.922×**).  Sprint 25's Day 9 sweep is the strongest evidence yet that the residual Pres_Poisson gap requires intervention at the FINEST FM level rather than at coarsening / intermediate-FM / coarsest-bisection — three independent algorithmic axes (HCC, multi-pass intermediate FM, spectral bisection) all wash out individually on this fixture.  Sprint 26 routing candidates (per `nd_tuning_day8.md` and `headline_summary.md`): (1) multi-pass FM at the FINEST level beyond Sprint 23's 3 passes (annealing acceptance / different bucket-tie-break / thick-restart-style FM with global rollback); (2) direct geometric cut detection on regular grids (Pres_Poisson is workload-specific but the headline); (3) per-vertex separator scoring akin to AMD pivot selection.
- Wall time on Pres_Poisson is ~47 s (Sprint 25 Day 11 5-run median; range 44 321 - 51 562 ms = 16.3 % within-run variance).  This is ~10 s above Sprint 24 Day 7's ~37 s reported measurement, but Day 11's per-phase profiling (`SPARSE_ND_PROFILE` instrumentation) classifies the drift as run-to-run measurement variance rather than algorithmic cost growth — the default ND code path is bit-identical between Sprint 23 and Sprint 25 (Sprint 24's 2 + Sprint 25's 3 new env vars are all default-off; nnz_L is bit-identical Sprint 23 → Sprint 25 across the corpus).  The variance is dominated by macOS arm64 thermal management on the partition phase (99.5 % of wall on this fixture).  See `docs/planning/EPIC_2/SPRINT_25/nd_wall_time_decision.md`.
