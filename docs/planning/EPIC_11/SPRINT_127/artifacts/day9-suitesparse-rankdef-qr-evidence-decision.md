# Sprint 127 Day 9 SuiteSparse Rank-Deficient QR Evidence Decision

## Decision

Day 9 explicitly defers SuiteSparse rank-deficient QR corpus evidence.

No checked-in SuiteSparse matrix satisfies the Day 8 metadata gate for a
rank-deficient QR fixture. Existing QR SuiteSparse matrices remain full-rank
controls, and the other checked-in, optional-large, and report-only matrices
lack independent expected-rank metadata, threshold/rank pairs, support-tier
promotion, runtime budget, and focused QR diagnostics required before
registration.
...
| Threshold semantics and absolute-threshold diagnostics | Defined as required by Day 8, but missing for any accepted corpus claim. |
| Runtime budget | Defined by Day 8 support-tier policy, but no candidate satisfies earlier metadata gates. |
| Optional-data skip behavior | Defined by Day 8 policy; no optional-large candidate is accepted. |
| Focused and full validation commands | Focused QR solve diagnostics run; no C or header changes, so full C gate is not required for Day 9. |
...
Day 9 ran the current QR solve executable to preserve control-matrix
diagnostics:
...
- `bcsstk04.mtx` remains a full-rank QR reconstruction, solve, and
  sparse-mode control.
- Sprint 125-127 dense and synthetic fixtures remain the source of bounded
...
- No minimum-norm, pseudoinverse, nullspace, subspace, Q-basis, economy,
  sparse-mode, reorder, backend, platform, performance, scalability, memory,
  package, ABI, public API, CI, CMake, or Makefile claim.
...
| --- | --- | --- |
| Project-plan Item 5 is complete or explicitly deferred. | Complete by deferral | No accepted SuiteSparse rank-deficient candidate satisfies the Day 8 metadata gate. |
| Accepted corpus evidence has deterministic skip and diagnostic behavior. | Not applicable | No candidate was accepted; future skip/diagnostic requirements remain in the promotion gate. |
| Broad corpus, platform, runtime, and performance claims remain absent. | Complete | See candidate review, evidence preserved, and non-claims. |
