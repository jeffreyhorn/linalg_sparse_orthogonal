...
def test_required_route_after_shorter_fence_line_stays_hidden() -> None:
    def mutate(root: Path) -> None:
        path = root / "README.md"
        path.write_text(
            path.read_text(encoding="utf-8").replace(
...
def test_required_route_in_raw_html_block_does_not_satisfy_contract() -> None:
    def mutate(root: Path) -> None:
        path = root / "README.md"
        path.write_text(
            path.read_text(encoding="utf-8").replace(
...
def test_reference_style_required_route_is_allowed() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        root = Path(temp_dir).resolve()
        copy_fixture(root)
        path = root / "README.md"
...
def test_case_variant_generated_html_link_fails_after_resolution() -> None:
    def mutate(root: Path) -> None:
        path = root / "docs" / "api_reference.md"
        path.write_text(
            path.read_text(encoding="utf-8") + "\n[Generated HTML](api/HTML/index.html)\n",
...
def main() -> None:
    ...
    test_required_route_in_raw_html_block_does_not_satisfy_contract()
    ...
    test_case_variant_generated_html_link_fails_after_resolution()
