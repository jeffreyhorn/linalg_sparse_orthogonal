...
**Characteristics:**
- Fill quality on regular meshes (default-path, **Sprint 27 production**): nos4 1.000× of AMD (n < threshold; entirely AMD); bcsstk04 1.184×; Kuu 1.882×; bcsstk14 1.124×; s3rmt3m3 1.028×; **Pres_Poisson 0.923× of AMD** (Sprint 22 Day 7 was 1.063×; the canonical 2D-PDE benchmark — ND beats AMD).  Sprint 27 Day 13's 24-setting × 6-fixture cross-corpus matrix (`bench_day13_combinations.csv`) confirms the production default is at-or-near the empirical floor on Pres_Poisson; no advisory combination beats it on the headline fixture.  Full corpus capture: `docs/planning/EPIC_2/SPRINT_23/bench_day12.txt` (default-path baseline through Sprint 23), `docs/planning/EPIC_2/SPRINT_25/bench_day9_combinations.csv` (Sprint 25 16-setting matrix), `docs/planning/EPIC_2/SPRINT_27/bench_day13_combinations.csv` (Sprint 27 24-setting matrix; current).
- Per-fixture advisory env-var settings (each documented in the cited decision docs).  **Sprint 27 Day 13's 24-setting × 6-fixture sweep** (`bench_day13_combinations.{csv,txt}`) is the current cross-corpus reference; the verified-best recipes per fixture-class:
    - **Pres_Poisson (headline)** — Sprint 27 production default (no env vars; HCC + t=128) at **0.923×** is itself the best across all 24 settings.  No advisory combination beats it; the closest contenders cluster at 0.927-0.944×.  Stacking advisory axes (especially `fixed_k×degree`) can REGRESS Pres_Poisson catastrophically (settings 5, 21, 23 land 1.65-2.09× — worse than AMD).  See `docs/planning/EPIC_2/SPRINT_27/headline_summary.md`.
...
- The Pres_Poisson default-path **0.923× ratio** is the cumulative effect of Sprint 23 Days 7 (leaf-AMD splice), 9-10 (gain-bucket FM lifting per-pass cost from O(n²) to O(|E|)), 11 (multi-pass FM at the finest level), Sprint 26 Day 5 (`nd_base_threshold 32 → 96`), Sprint 27 Day 2 (`SPARSE_ND_COARSENING heavy_edge → hcc` with Kuu-safe degree-CV-fall-through), and Sprint 27 Day 3 (`nd_base_threshold 96 → 128`).  PLAN.md's literal **≤ 0.85× target across Sprints 24-27 remains UNMET** (5 consecutive sprints; current default 0.923× = 7.3pp from target; per-sprint trajectory: 0.952× → 0.942× → 0.922× → 0.9217× → 0.923×).  Sprint 27's structural-pipeline-level interventions all REGRESSED Pres_Poisson on the headline fixture: Item 4 (annealing FM) +2.2-3.1pp, Item 5 (root-spectral) +2.3pp, Item 6 (thick-restart FM) +4.7-11.5pp.  The Sprint 27 Day 13 cross-corpus matrix confirms the multilevel pipeline + leaf-AMD reaches near-optimal cuts on this fixture that pipeline-level interventions can't improve — the literal 0.85× target appears to be at or below the empirical floor for ND-style algorithms on this fixture-class.  Routing to Sprint 28+ for non-pipeline-level interventions per PROJECT_PLAN.md follow-up (e.g. METIS-style multi-matchings coarsening, geometric-aware domain decomposition, or supernodal reordering on the elimination tree).
