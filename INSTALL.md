...
| Windows selected Cholesky comparison workflow | guarded-workflow: source-controlled workflow path exists, but support/freshness promotion remains blocked on evidence and metadata review | Windows selected comparison workflow and selected target manifest | bounded selected Cholesky comparison freshness workflow, `normalize_report_index.py --selected-target cholesky-spd-tridiag-5`, PowerShell guard; selected target manifest promotion remains separate | No promoted Windows selected freshness until hosted evidence, selected metadata, support tier, and claim contract are reviewed together; no broad Windows report freshness, Windows selected oracle freshness, Windows selected benchmark freshness, or unselected Windows comparison families. |
