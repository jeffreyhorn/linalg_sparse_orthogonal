...
def test_claim_boundary_windows_qr_selected_promotion_wording_fails_clearly() -> None:
    path = validator.REPO_ROOT / "README.md"
    text = path.read_text(encoding="utf-8") + (
        "\nWindows QR incompatible selected freshness is promoted.\n"
    )
    assert_raises_with(
        lambda: validator.validate_claim_boundaries({path: text}),
        "unsupported Windows/PowerShell claim",
...
if __name__ == "__main__":
    ...
    test_claim_boundary_windows_qr_selected_promotion_wording_fails_clearly()
