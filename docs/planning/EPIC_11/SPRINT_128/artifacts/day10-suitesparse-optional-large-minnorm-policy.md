# Sprint 128 Day 10 SuiteSparse And Optional-Large Minimum-Norm Policy

## Decision

Day 10 defines the Sprint 128 SuiteSparse and optional-large minimum-norm gate.
It does not add new corpus evidence today.

The accepted SuiteSparse minimum-norm corpus baseline remains the bounded
Sprint 125 `west0067.mtx` first-30-row smoke: a `30 x 67` underdetermined
system with `b = A * ones`, residual below `1e-8`, positive solution norm, and
`||x_min|| <= ||ones|| + 1e-8`. Sprint 128 must not relabel that smoke as
broad SuiteSparse, optional-large, rank-deficient SuiteSparse, platform,
performance, or QR-vs-SVD corpus evidence.
...
tolerance, support-tier, skip, runtime, platform, and duplicate-fence rules
below. Otherwise Day 11 must explicitly defer Project Plan Item 6's corpus
expansion.
...
| Input | Role |
| --- | --- |
| Sprint 128 Plan Day 10 | Defines the SuiteSparse and optional-large minimum-norm policy deliverables. |
| Sprint 128 Day 8-9 artifacts | Keep SuiteSparse rank-deficient QR corpus evidence deferred without independent rank metadata. |
...
| Sprint 126 Day 10-11 artifacts | Define the prior SuiteSparse minimum-norm metadata protocol and explicit deferral. |
| Sprint 127 Day 10-11 artifacts | Define the prior optional-large gate and explicit deferral. |
...
| Lane | Owner | Status | Sprint 128 interpretation |
| --- | --- | --- | --- |
...
| Exact `3 x 6` minimum-norm | `tests/test_colamd.c`, `tests/qr_external_dense_reference.py` | Complete | Additional exact-value lane only; not corpus evidence. |
...
## Accepted Baseline Details

| Field | Value |
| --- | --- |
| Matrix path | `tests/data/suitesparse/west0067.mtx` |
| Extraction | First 30 rows copied into a `30 x 67` underdetermined system. |
| RHS | `b = A * ones`, where `ones` has length `67`. |
| Residual metric | Absolute maximum row residual `max_i |(A*x)_i - b_i|`. |
| Residual tolerance | `< 1e-8`. |
| Norm metric | `||x_min||_2` compared with `||ones||_2`. |
| Norm tolerance | `||x_min||_2 <= ||ones||_2 + 1e-8`, with positive `||x_min||_2`. |
| Support tier | Default checked-in data. Missing `west0067.mtx` is a default-data failure for the suite; the current test prints skip-style text on load or solve failure but is treated as legacy smoke behavior, not optional-large policy. |
| Claim boundary | One corpus submatrix smoke only. No broad SuiteSparse, optional-large, rank-deficient, platform, runtime, or performance claim. |
...
| SuiteSparse QR-vs-SVD minimum-norm corpus cross-check | Any bounded corpus extraction | Depends on selected matrix | Candidate only after QR and SVD roles, tolerances, runtime, support tier, and non-oracle wording are pinned. |
...
## Support-Tier, Skip, Runtime, And Platform Policy

| Tier | Policy |
| --- | --- |
| Default checked-in | Missing data is a failure. Accepted numerical disagreement is a failure. Added default runtime must fit normal `make test`. |
...
optional-data, or public wording files changed for Day 10, so no new code
...
| --- | --- | --- |
| No minimum-norm candidate proceeds without pinned extraction and metrics. | Complete | Metadata protocol, diagnostics/tolerance rules, and candidate table gate Day 11. |
| Optional-large behavior is deterministic and explicitly gated. | Complete | Optional-large candidates require `SPARSE_TEST_LARGE=1`, skip proof, support-tier notes, and runtime budget before acceptance. |
| Accepted evidence cannot imply broad SVD-pseudoinverse parity. | Complete | SVD is allowed only as a bounded cross-check with owner and non-oracle wording. |
