// sparse_lu_orthogonal_complete_pivoting_mm_full.c
// ~600 lines – complete orthogonal sparse LU with complete pivoting
// + node pool, nnz counter, memory estimator, Matrix Market I/O
// + full error checking + unit tests for error paths
// Compile: gcc -O2 -Wall -o sparse_lu sparse_lu_orthogonal_complete_pivoting_mm_full.c -lm
...
// ──────────────────────────────────────────────────────────────────────────────
// Core operations with error checking
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// Complete pivoting LU – full implementation
// ──────────────────────────────────────────────────────────────────────────────
...
int computeLU(SparseMatrix* mat, double tol) {
    ...
    for (idx_t k = 0; k < n; k++) {
        ...
        // Search submatrix [k:n, k:n] for largest absolute value
        for (idx_t log_j = k; log_j < n; log_j++) {
            idx_t phys_j = mat->col_perm[log_j];
            Node* curr = mat->colHeaders[phys_j];
            while (curr) {
                idx_t phys_i = curr->row;
        ...
        if (max_val < tol) {
            fprintf(stderr, "Near-singular at step %" PRId32 " (pivot < %g)\n", k, tol);
            return ERR_SINGULAR;
        }

        // Swap rows
        if (pivot_log_row != k) {
            idx_t temp = mat->row_perm[k];
            mat->row_perm[k] = mat->row_perm[pivot_log_row];
            mat->row_perm[pivot_log_row] = temp;
            mat->inv_row_perm[mat->row_perm[k]] = k;
            mat->inv_row_perm[mat->row_perm[pivot_log_row]] = pivot_log_row;
        }

        // Swap columns
        if (pivot_log_col != k) {
            idx_t temp = mat->col_perm[k];
            mat->col_perm[k] = mat->col_perm[pivot_log_col];
            mat->col_perm[pivot_log_col] = temp;
            mat->inv_col_perm[mat->col_perm[k]] = k;
            mat->inv_col_perm[mat->col_perm[pivot_log_col]] = pivot_log_col;
        }

        // Elimination step
        idx_t phys_k = mat->col_perm[k];
        Node* curr = mat->colHeaders[phys_k];
        while (curr) {
            idx_t phys_r = curr->row;
            idx_t log_i = mat->inv_row_perm[phys_r];
            if (log_i > k) {
                double pivot = getValue(mat, k, k);
                if (fabs(pivot) < tol) return ERR_SINGULAR;
                double mult = getValue(mat, log_i, k) / pivot;
                if (setValue(mat, log_i, k, mult) != ERR_OK) return ERR_MALLOC_FAIL;

                idx_t phys_row_k = mat->row_perm[k];
                Node* uj = mat->rowHeaders[phys_row_k];
                while (uj) {
                    idx_t phys_j = uj->col;
                    idx_t log_j = mat->inv_col_perm[phys_j];
                    if (log_j > k) {
                        double new_val = getValue(mat, log_i, log_j) - mult * uj->value;
                        if (fabs(new_val) < DROP_TOL * max_val) {
                            if (setValue(mat, log_i, log_j, 0.0) != ERR_OK) return ERR_MALLOC_FAIL;
                        } else {
                            if (setValue(mat, log_i, log_j, new_val) != ERR_OK) return ERR_MALLOC_FAIL;
                        }
                    }
                    uj = uj->right;
                }
...
int main(void) {
    ...
    // Optional: small working example
    ...
    int err = computeLU(mat, 1e-10);
    if (err != ERR_OK) {
        fprintf(stderr, "LU failed: error %d\n", err);
        freeSparseMatrix(mat);
        return 1;
    }
    ...
    err = solveLU(mat, b, x);
    if (err != ERR_OK) {
        fprintf(stderr, "Solve failed: error %d\n", err);
    } else {
        printf("\nSolution x = [%.6f, %.6f, %.6f]\n", x[0], x[1], x[2]);
    }
