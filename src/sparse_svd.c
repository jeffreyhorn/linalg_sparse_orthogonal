...
/* Optional outer-product accumulator for `sparse_svd_lowrank_sparse`.
 * See `docs/planning/EPIC_2/SPRINT_29/lowrank_design_day1.md` for the
 ...
 * Memory: O(nnz_result) sparse output only.  Wall: O(m·n·rank_k) same as
 * the dense-intermediate path's accumulator-fill loop.
 *
 * Env-var gate `SPARSE_SVD_LOWRANK_OUTER={off (default), on}` lets
 * callers A/B compare the two paths; the recorded sweep verdict
...
typedef enum {
    ...
    SVD_LOWRANK_OUTER_ON = 1,  /* Outer-product accumulator */
...
sparse_err_t sparse_svd_extract_uv(const sparse_bidiag_t *bd, double *U, double *V) {
    ...
    if (bd->transposed) {
        ...
        if (sparse_idx_to_size_checked(mt, &mt_size) || sparse_idx_to_size_checked(nt, &nt_size) ||
        ...
        if (sparse_size_mul_overflow(mt_size, k_size, &mt_k_elems) ||
            sparse_size_mul_overflow(mt_k_elems, sizeof(double), &mt_k_bytes) ||
            sparse_size_mul_overflow(nt_size, k_size, &nt_k_elems) ||
        ...
        if (V) { /* A's V = A^T's U_t */
            ...
            for (idx_t i = 0; i < k; i++)
                Ut[(size_t)i * mt_size + (size_t)i] = 1.0;
            ...
            for (idx_t j = 0; j < k; j++) {
                for (idx_t r = k - 1; r >= 0; r--) {
                    if (bd->u_betas[r] == 0.0)
                        continue;
                    idx_t len = mt - r;
                    hh_apply(bd->u_vecs[r], bd->u_betas[r], &Ut[(size_t)j * mt_size + (size_t)r],
                             len);
                }
            }
        ...
        if (U) { /* A's U = A^T's V_t */
            ...
            for (idx_t i = 0; i < k; i++)
                Vt[(size_t)i * nt_size + (size_t)i] = 1.0;
            ...
            for (idx_t j = 0; j < k; j++) {
                for (idx_t r = nv - 1; r >= 0; r--) {
                    ...
                    hh_apply(bd->v_vecs[r], bd->v_betas[r],
                             &Vt[(size_t)j * nt_size + (size_t)(r + 1)], len);
                }
            }
    ...
    size_t mk_bytes, nk_bytes;
    {
        ...
        if (sparse_size_mul_overflow(m_size, k_size, &mk_elems) ||
            sparse_size_mul_overflow(mk_elems, sizeof(double), &mk_bytes) ||
            sparse_size_mul_overflow(n_size, k_size, &nk_elems) ||
    ...
    if (U) {
        memset(U, 0, mk_bytes);
        for (idx_t i = 0; i < k; i++)
            U[(size_t)i * m_size + (size_t)i] = 1.0;
        ...
        for (idx_t j = 0; j < k; j++) {
            ...
            for (idx_t r = k - 1; r >= 0; r--) {
                if (bd->u_betas[r] == 0.0)
                    continue;
                idx_t len = m - r;
                hh_apply(bd->u_vecs[r], bd->u_betas[r], &U[(size_t)j * m_size + (size_t)r], len);
            }
        }
    }
    ...
    if (V) {
        ...
        memset(V, 0, nk_bytes);
        for (idx_t i = 0; i < k; i++)
            V[(size_t)i * n_size + (size_t)i] = 1.0;
        ...
        for (idx_t j = 0; j < k; j++) {
            ...
            for (idx_t r = nv - 1; r >= 0; r--) {
                if (bd->v_betas[r] == 0.0)
                    continue;
                idx_t len = n - r - 1;
                hh_apply(bd->v_vecs[r], bd->v_betas[r], &V[(size_t)j * n_size + (size_t)(r + 1)],
                         len);
            }
        }
    }
...
sparse_err_t sparse_svd_compute(const SparseMatrix *A, const sparse_svd_opts_t *opts,
    ...
    /* Full-mode (`compute_uv && !economy`) extends U from m×k to m×m and
     * Vt from k×n to n×n with a post-iteration MGS pad step. Economy mode
     * keeps the compact basis only. Storage contract: column-major with
     * leading dim = number of rows. See
     ...
     * (operates on the economy k columns of U_work + V_work; the
     * column-permutation on V_work and the row-permutation on
     * Vt = transpose(V_work) describe the same singular-triplet
     * permutation). Done before full-mode padding so the padded columns
     * (cols k..m-1 of U, cols k..n-1 of V) are added in an unambiguous
     * orientation. */
...
sparse_err_t sparse_svd_lowrank_sparse(const SparseMatrix *A, idx_t rank_k, double drop_tol,
    ...
    *result = NULL;
    ...
    /* Optional outer-product path that drops the O(m·n) dense accumulator.
     * The default-off path preserves the dense-intermediate behavior;
     * env-on dispatches to `sparse_svd_lowrank_outer_product`, which
     * matches the dense path's nnz output bit-for-bit on the corpus, with
     * -76-88 % peak
