...
static double relative_residual(const SparseMatrix *A, const double *b,
                                const double *x, idx_t n) {
...
static double block_relative_residual(const SparseMatrix *A, const double *B,
...
static void make_rhs(const SparseMatrix *A, double *b, double *x_exact,
                     idx_t n) {
    for (idx_t i = 0; i < n; i++)
...
static void make_block_rhs(const SparseMatrix *A, double *B, double *X_exact,
                           idx_t n, idx_t nrhs) {
    for (idx_t k = 0; k < nrhs; k++)
        for (idx_t i = 0; i < n; i++)
...
static void test_csr_solve_nos4(void) {
    csr_solve_matrix(SS_DIR "/nos4.mtx", "nos4");
}

static void test_csr_solve_bcsstk04(void) {
    csr_solve_matrix(SS_DIR "/bcsstk04.mtx", "bcsstk04");
}

static void test_csr_solve_west0067(void) {
    csr_solve_matrix(SS_DIR "/west0067.mtx", "west0067");
}

static void test_csr_solve_steam1(void) {
    csr_solve_matrix(SS_DIR "/steam1.mtx", "steam1");
}

static void test_csr_solve_fs_541_1(void) {
    csr_solve_matrix(SS_DIR "/fs_541_1.mtx", "fs_541_1");
}

static void test_csr_solve_orsirr_1(void) {
    csr_solve_matrix(SS_DIR "/orsirr_1.mtx", "orsirr_1");
}
...
static void test_block_solvers_cross_validate_nos4(void) {
    ...
    ASSERT_ERR(sparse_cg_solve_block(A, B, nrhs, X_cg, &cg_opts, NULL, NULL,
                                     &cg_res),
               SPARSE_OK);
    ...
    sparse_gmres_opts_t gm_opts = {
        .max_iter = 2000, .restart = 50, .tol = 1e-12};
    sparse_iter_result_t gm_res;
    ASSERT_ERR(sparse_gmres_solve_block(A, B, nrhs, X_gmres, &gm_opts, NULL,
                                        NULL, &gm_res),
               SPARSE_OK);
...
static void test_preconditioned_block_gmres_steam1(void) {
    ...
    sparse_gmres_opts_t opts = {
        .max_iter = 1000, .restart = 50, .tol = 1e-10};
    sparse_iter_result_t res;
    ASSERT_ERR(sparse_gmres_solve_block(A, B, nrhs, X, &opts,
                                        sparse_ilu_precond, &ilu, &res),
               SPARSE_OK);

    double rr = block_relative_residual(A, B, X, n, nrhs);
    printf("    steam1 (%d×%d, %d RHS): ILU+GMRES relres=%.2e iters=%d\n", n,
           n, nrhs, rr, res.iterations);
    ASSERT_TRUE(rr < 1e-6);
