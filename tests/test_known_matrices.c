...
{
    SparseMatrix *A = NULL;
    if (sparse_load_mm(&A, path) != SPARSE_OK) return -1.0;

    idx_t n = sparse_rows(A);
    if (n != sparse_cols(A)) { sparse_free(A); return -2.0; }
...
{
    double res = load_solve_residual(DATA_DIR "/identity_5.mtx",
...
{
    double res = load_solve_residual(DATA_DIR "/diagonal_10.mtx",
                                     SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_residual(DATA_DIR "/tridiagonal_20.mtx",
                                     SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_refine_residual(DATA_DIR "/tridiagonal_20.mtx",
                                            SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_residual(DATA_DIR "/symmetric_4.mtx",
                                     SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_residual(DATA_DIR "/bcsstk01.mtx",
                                     SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_refine_residual(DATA_DIR "/bcsstk01.mtx",
                                            SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
...
{
    double res = load_solve_residual(DATA_DIR "/unsymm_5.mtx",
                                     SPARSE_PIVOT_COMPLETE);
    ASSERT_TRUE(res >= 0.0);
