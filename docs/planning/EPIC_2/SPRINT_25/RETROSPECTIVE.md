...
| 2. Multi-pass FM at intermediate uncoarsening levels | partial | Day 4-5 commits `72a12c4` / `5b4811c`; `SPARSE_FM_INTERMEDIATE_PASSES` env var (default 1, range [1, 10]); Pres_Poisson sweep ∈ {1, 2, 3}: passes=2 essentially unchanged (-0.04pp); passes=3 regresses +1.5pp (0.952× → 0.967×); default stays at 1 because the PLAN.md flip rule fails on Pres_Poisson, but per-fixture wins exist (Kuu passes=3 -23.2pp; bcsstk14 passes=2 -2.2pp); s3rmt3m3 regresses +1.7pp at passes ≥ 2 (`intermediate_fm_decision.md`) |
...
| 5 (`5b4811c`) | `SPARSE_FM_INTERMEDIATE_PASSES` sweep + decision | passes ∈ {1, 2, 3} sweep on Pres_Poisson; passes=2 = 0.952× (essentially unchanged, -0.04pp); passes=3 = 0.967× (+1.5pp regression); `intermediate_fm_decision.md` defaults stay at 1 because Pres_Poisson flip rule fails at both candidate values; per-fixture wins recorded for Kuu (passes=3 -23.2pp; the strongest single-fixture single-axis win Sprint 25 produced) and bcsstk14 (passes=2 -2.2pp); smoke test `test_fm_intermediate_passes_smoke` lands |
