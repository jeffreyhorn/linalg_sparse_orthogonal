...
| SVD helper header | `tests/test_svd_helpers.h` owns only selected rank, pseudoinverse, and dense low-rank helper implementations. | No code change needed. |
| Guard script | `scripts/check_svd_helper_guard.sh` checks required files, proof-owner registration, helper include boundary, selected marker movement, and header-only non-registration. | No code change needed. |
| Guard regression test | `tests/test_svd_helper_guard.py` includes fixture-positive and drift-negative cases for missing include, missing dependency include, duplicate moved ownership, missing registrations, and accidental helper registration. | No code change needed. |
| Build registration | `Makefile` exposes `svd-helper-guard`; `CMakeLists.txt` and `build-metadata/library_sources.txt` are intentionally unchanged. | No code change needed. |
...
| --- | --- | --- |
| Selected moved bodies remain owned by `tests/test_svd_helpers.h`, not `tests/test_svd.c`. | `make svd-helper-guard`; `python3 tests/test_svd_helper_guard.py`; moved marker checks in `scripts/check_svd_helper_guard.sh`. | Covered. |
| Selected `RUN_TEST(...)` registrations stay in the `tests/test_svd.c` proof-owner binary exactly once. | `make svd-helper-guard`; missing-registration negative fixture in `tests/test_svd_helper_guard.py`; focused `./build/test_svd`. | Covered. |
| Helper dependency includes stay explicit and local to the helper header. | `make svd-helper-guard`; missing `sparse_svd.h` and `sparse_vector.h` negative fixtures. | Covered. |
| Helper remains header-only and is not registered as a standalone build/test/library source. | `make svd-helper-guard`; helper Makefile and library-manifest negative fixtures; Day 12 `make source-list-check`; Day 12 CMake configure check. | Covered. |
...
| Public API, ABI, solver behavior, numerical tolerances, package support, platform support, performance, and state-of-art claims are not promoted by this sprint. | `git diff --name-only -- include src CMakeLists.txt build-metadata/library_sources.txt .github`; maintainer/project-plan/residual-queue non-claims. | Covered as non-claims. |
...
- One selected SVD review surface was reduced by moving the rank,
  ...
  `tests/test_svd_helpers.h`, while retaining `tests/test_svd.c` as the
  proof-owner binary.
...
- public documentation, package, platform, performance, release, or
  state-of-art support promotion.
