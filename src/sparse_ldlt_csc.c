...
sparse_err_t ldlt_csc_detect_supernodes(const LdltCsc *F, idx_t min_size, idx_t *super_starts,
    ...
    while (j < n) {
        /* Skip a column if it's the second of a 2x2 pair; the prior
         * iteration already decided what to do with the pair, and supernodes
         * must not start in the middle of an atomic pivot block. */
...
sparse_err_t ldlt_csc_from_sparse(const SparseMatrix *mat, const idx_t *perm_in, double fill_factor,
    ...
    *ldlt_out = NULL;
    ...
    m->pivot_size = calloc(alloc_n, sizeof(idx_t));
    m->perm = calloc(alloc_n, sizeof(idx_t));
    /* Row-adjacency index: same calloc-zero initial state as
     * `ldlt_csc_alloc` so cmod updates can read row_adj_count[col] == 0 and
     * short-circuit before any column has been factored. */
    m->row_adj = calloc(alloc_n, sizeof(idx_t *));
    m->row_adj_count = calloc(alloc_n, sizeof(idx_t));
    m->row_adj_cap = calloc(alloc_n, sizeof(idx_t));
    if (!m->D || !m->D_offdiag || !m->pivot_size || !m->perm || !m->row_adj || !m->row_adj_count ||
     ...
     * else identity.  Bunch-Kaufman pivoting composes further
 ...
 * `chol_csc_from_sparse_with_analysis` for the LDL^T side. It pre-allocates
 * the embedded `L` with
 * every column's full sym_L pattern rather than the heuristic
 * `fill_factor × A.nnz` pattern that `ldlt_csc_from_sparse` produces.
 * This closes the indefinite-fill hole covered by the direct CSC regression
 * tests: without the full symbolic pattern, batched
 * `ldlt_csc_eliminate_supernodal` writeback can silently drop cmod fill rows
 * on KKT-style saddle points, producing residuals far above round-off.
 *
 ...
 *   approaches considered for the conversion workflow were:
 ...
 *   Option D is the maintained behavior: transparent dispatch wraps the
 *   pre-pass workflow behind `sparse_ldlt_factor_opts` so public-API callers
 *   never see it, while batched-test helpers use the same two-pass structure.
 *
 * Implementation:
 ...
 *      already validated on the direct CSC corpus.
 ...
 *
 * The resulting `L->sym_L_preallocated == 1` state drives
 * `ldlt_csc_eliminate_supernodal`'s writeback fast path so indefinite KKT
 * fixtures remain at round-off residuals. */
...
sparse_err_t ldlt_csc_writeback_to_ldlt(const LdltCsc *F, double tol, sparse_ldlt_t *ldlt_out) {
     ...
     * the dense factor owner and the CSC elimination kernels).  The
     * `v == 0.0` filter below therefore never drops the diagonal in
     * practice, and the `i != j` guard on the below-diagonal threshold
     * keeps the unit diagonal from being accidentally filtered by the
     * drop_tol test. */
...
/* ═══════════════════════════════════════════════════════════════════════
 * Native CSC Bunch-Kaufman scaffolding
 * ═══════════════════════════════════════════════════════════════════════ */
...
/* ─── Native kernel: full 1x1 and 2x2 pivots ─────────────────────── */

/* One native column step is extracted so `ldlt_csc_eliminate_supernodal` can
 * interleave it with the batched supernodal path on non-supernodal columns.
 *
...
/* ═══════════════════════════════════════════════════════════════════════
 * LDL^T solve: forward / diagonal-block / backward sweeps
 * ═══════════════════════════════════════════════════════════════════════ */
 ...
 * See the design block at the top of this file and the doc-comments on
 * declarations in
