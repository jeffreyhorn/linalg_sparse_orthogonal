...
Comparison Families, Package Boundaries, Performance Methodology & Final Claim
Calibration
**Sprints:** 157-166
...
public-header/API coherence, static-first package boundary hardening, and final
claim recalibration.
...
| 166 | Reconciled final evidence, local validation, hosted CI scope, public claims, project-plan item status, success criteria, final residuals, and Epic 14 closeout material. |
...
| --- | --- | --- |
| Sprint 164 public-header gate | Passed `make format && make lint && make test`; declaration checksum matched baseline and `make docs-check` passed. | Selected public-header cleanup only, not broad API redesign. |
| Sprint 165 package validation | Passed full quality gate, static package deferral, Make install/`pkg-config`, CMake install/export, and package report-index checks. | Static-first package proof only. |
...
| Sprint 166 Day 11 project-plan reconciliation II | Sprints 162-166 items and Epic 14 success criteria were reconciled with evidence-backed statuses. | Final Sprint 166 hosted CI confirmation remains PR-time evidence. |
| Sprint 166 Day 14 final docs checks | `git diff --check` and targeted claim scans passed. | Documentation and workflow closeout hygiene; no new C/header gate required for Day 14. |

## Earned Claims

Epic 14 earns these claims with qualifiers:

- Generated API reference publication is no longer ambiguous: generated HTML
  is intentionally ignored/local-only, while checked-in public headers and
...
The final Sprint 166 residual queue is published in
[SPRINT_166/artifacts/day13-final-residual-queue-and-closeout-prep.md](./SPRINT_166/artifacts/day13-final-residual-queue-and-closeout-prep.md).

Highest-priority next-epic candidates:

| Priority | Candidate | Closure target |
| --- | --- | --- |
| 1 | Sprint 166 PR-hosted CI confirmation | Confirm Linux/macOS/Windows PR results and update closeout evidence if hosted behavior differs from local validation. |
...
| 3 | Shared-library ABI product design | Decide whether to continue static-only support or fund symbol visibility, export/import, SONAME/install-name/DLL metadata, runtime-loader validation, and ABI policy. |
| 4 | Package-manager distribution readiness | Define selected package-manager scope, provenance, install layout, versioning, CI proof, and support-tier wording before claiming ecosystem distribution. |
| 5 | Broader public-header cleanup batch | Apply the Sprint 164 declaration-preserving process to remaining high-risk public headers such as QR, SVD, ILU, IC, and LDLT. |
| 6 | Additional bounded comparison family | Add one selected fixture-local family with source-controlled references, metrics, normalized rows, tests, and clear non-parity wording. |

Long-horizon deferrals remain broad external-library parity, portable
performance superiority, broad state-of-the-art positioning, broad
cross-platform generated-report parity, Windows Makefile parity, Windows
`pkg-config` execution parity, hosted generated API HTML publication, and
package-manager/provider upgrade behavior.
...
1. **Product decisions closed real ambiguity.** Sprints 158, 162, and 165
   chose local-only generated HTML, CMake-first Windows package support, and
   static-first package boundaries instead of silently implying unsupported
   surfaces.

2. **Hosted evidence was reconciled after selected-family growth.** Sprint 166
   Day 7 corrected the hosted comparison artifact scope after Sprint 160 and
   Sprint 161 expanded selected comparison families.

3. **Comparison work stayed useful because it stayed narrow.** QR and
   partial-SVD comparison additions name exact fixtures, references, metrics,
   tolerances, row IDs, and non-claims.

4. **Generated report support tiers became clearer.** Local generated rows,
   source-controlled metadata, advisory rows, selected required rows, and
   hosted artifacts are now described as different evidence classes.

5. **Declaration-preserving header cleanup kept review risk controlled.**
   Sprint 164 improved API usability without drifting signatures.

6. **Static-first packaging is more honest.** Shared-library, ABI,
   runtime-loader, package-manager, and Windows parity non-claims are guarded
   instead of implied.

## Could Be Better

1. **Hosted evidence still arrives late in the lifecycle.** Sprint 166 can
   validate local workflow structure, but branch-level hosted CI evidence only
   exists after the PR runs.

2. **Generated API HTML remains intentionally unhosted.** The ambiguity is
   closed, but discoverability is still weaker than a hosted generated API
   reference would be.

3. **Performance publication is local-only.** Methodology metadata improved,
   but there is no reviewed hosted performance publication or superiority
   methodology.

4. **Package and ABI scope remains intentionally conservative.** Static-first
   proof is stronger, but shared-library support, dynamic ABI policy,
   runtime-loader behavior, and package-manager distribution remain future
   product work.

5. **Header cleanup is still partial.** The selected Sprint 164 batch was
   declaration-preserving and useful, but several public headers still need the
   same treatment.
...
- [SPRINT_166/artifacts/day5-local-validation-baseline.md](./SPRINT_166/artifacts/day5-local-validation-baseline.md)
- [SPRINT_166/artifacts/day6-supplemental-validation-sweep.md](./SPRINT_166/artifacts/day6-supplemental-validation-sweep.md)
- [SPRINT_166/artifacts/day7-hosted-ci-evidence-reconciliation.md](./SPRINT_166/artifacts/day7-hosted-ci-evidence-reconciliation.md)
- [SPRINT_166/artifacts/day8-public-claim-audit-performance-report.md](./SPRINT_166/artifacts/day8-public-claim-audit-performance-report.md)
- [SPRINT_166/artifacts/day9-public-claim-audit-package-abi-windows.md](./SPRINT_166/artifacts/day9-public-claim-audit-package-abi-windows.md)
- [SPRINT_166/artifacts/day10-project-plan-reconciliation-part1.md](./SPRINT_166/artifacts/day10-project-plan-reconciliation-part1.md)
- [SPRINT_166/artifacts/day11-project-plan-reconciliation-part2.md](./SPRINT_166/artifacts/day11-project-plan-reconciliation-part2.md)
- [SPRINT_166/artifacts/day13-final-residual-queue-and-closeout-prep.md](./SPRINT_166/artifacts/day13-final-residual-queue-and-closeout-prep.md)

## Completion

Epic 14 is complete with explicit residuals. The completed work improves
evidence discipline, hosted selected generated evidence, comparison-family
coverage, static-first package confidence, generated API policy clarity,
methodology-bound performance reporting, public-header coherence, and public
claim hygiene. The remaining work is visible, prioritized, and bounded by
promotion gates rather than implied by existing evidence.
