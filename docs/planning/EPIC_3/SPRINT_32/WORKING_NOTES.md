...
Day 11 applied the Day 10 Batch I rule exactly:

- `return INFINITY;` -> `return HUGE_VAL;`
- `double ... = INFINITY;` -> `double ... = HUGE_VAL;`

Representative cleanup patterns:

- residual helpers now return `HUGE_VAL` on allocation or intermediate failure
- local sentinel initializers in the Sprint 6 and Sprint 18 integration helpers now use `HUGE_VAL`
- the Sprint 20 helper comment now names the actual sentinel returned by the code
- the BiCGSTAB helper comment was tightened to describe the actual purpose of the `HUGE_VAL` sentinel rather than the old `1.0/0.0` fallback narrative

No test contract or tolerance changed. This was a warning-only mechanical cleanup.
...
- direct runs:
  ...
  - `./build/test_sprint19_integration`
  - `./build/test_sprint20_integration`
- `cmake --build build/sprint32-day1-cmake --parallel 1 --clean-first > /tmp/sprint32_day11_build.stdout 2> /tmp/sprint32_day11_build.stderr`
...
- The helper-sentinel batch closed exactly as designed.
- No non-SVD `-Wdouble-promotion` sites remain in Sprint 32 scope.
- Day 12 is now a single-file cleanup plus warning reconciliation:
  - `tests/test_svd.c`
  - `6` residual `-Wdouble-promotion` sites
