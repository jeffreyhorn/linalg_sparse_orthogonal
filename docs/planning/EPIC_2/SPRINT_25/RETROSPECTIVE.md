...
| 2. Multi-pass FM at intermediate uncoarsening levels | partial | Day 4-5 commits `72a12c4` / `5b4811c`; `SPARSE_FM_INTERMEDIATE_PASSES` env var (default 1); passes ∈ {1,2,3,5,10} sweep saturates at 0.952× on Pres_Poisson; passes=2 advisory for s3rmt3m3; default unchanged (`intermediate_fm_decision.md`) |
...
| 5 (`5b4811c`) | `SPARSE_FM_INTERMEDIATE_PASSES` sweep + decision | passes ∈ {1, 2, 3, 5, 10} sweep on Pres_Poisson; passes=2 = 0.953× (+0.1pp regression); passes=3 = 0.952× (unchanged); passes ≥ 5 saturate; `intermediate_fm_decision.md` defaults stay at 1; passes=2 advisory for s3rmt3m3; smoke test `test_fm_intermediate_passes_smoke` lands |
