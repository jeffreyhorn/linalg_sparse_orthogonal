...
- **Progress / cancel callbacks** (Sprint 29 Days 6-7) — `sparse_progress_cb_t` + `opts->progress_cb` / `opts->progress_user` across LU, Cholesky, LDL^T (linked-list + CSC), QR, CG, GMRES, MINRES, BiCGSTAB, Lanczos, and LOBPCG.  Callback signature emits `phase` / `step` / `total` / `elapsed_s`; a non-zero return cancels with `SPARSE_ERR_CANCELLED`.  In-place factor routines (LU / Cholesky / LDL^T) may have already cached `factor_norm`, cleared `factored`, or stripped the upper triangle (Cholesky) by the time the first callback fires, so cancellation does not guarantee a bit-identical input matrix — callers needing that should `sparse_copy()` the input before factoring.  See `include/sparse_lu.h` / `include/sparse_cholesky.h` opts headers for the per-routine contract.  Default `NULL` callback runs at zero overhead (no `make wall-check` regression vs Sprint 28).
