...
- `scripts/homebrew_local_formula_proof.sh` is the selected Sprint 180
  ...
  local source archive, renders the formula through a temporary local tap,
  installs from source, validates the installed static package surface, runs
  the downstream `brew test`, uninstalls, cleans up, and reached proof exit `0`
  in the Sprint 198 developer-mode macOS Intel x86_64 Tier 3 Homebrew run. Do
  not cite that as Homebrew/core readiness, bottle support, Linuxbrew support,
  public tap maintenance, binary package distribution, or broad
  package-manager support. Placeholder values such as `NOASSERTION`,
  `UNKNOWN`, `TBD`, `TODO`, or template placeholder text remain blocker
  evidence rather than package proof. Interpret exit `0` as local static source
  formula proof only, exit `2` as unavailable/blocker evidence, and any other
  nonzero exit as a proof failure.
- Sprint 198 records the current Homebrew proof runbook status in
  `docs/planning/EPIC_18/SPRINT_198/WORKING_NOTES.md`. Maintainers may cite
...
- `tests/test_etree.c` owns the Sprint 195 bounded allocation-failure lane
  for selected `sparse_symbolic_cholesky()` output construction and the Sprint
  200 bounded allocation-failure lane for the selected `sparse_symbolic_lu()`
  owner:
  ...
  - `test_symbolic_lu_allocation_failures_clear_outputs`
  - `test_symbolic_lu_allocation_failures_cleanup_sweep`
  - `test_symbolic_lu_allocation_failures_recover_on_retry`
  - maintained focused Make command: `make symbolic-allocation-failure-gate`
  - maintained selected symbolic LU Make command:
  ...
  - maintained selected symbolic LU registration guard:
  ...
  - Sprint 200 invariant and gate evidence:
    `docs/planning/EPIC_18/SPRINT_200/artifacts/day4-invariant-record.md`
    and
    `docs/planning/EPIC_18/SPRINT_200/artifacts/day10-focused-gate.md`
  - maintained symbolic Cholesky proof stays local to selected output
    allocation, partial-state cleanup, stale-output suppression, repeated
    cleanup after failure, and retry-after-reset behavior on bounded fixtures
  - maintained symbolic LU proof stays local to selected
    `sparse_symbolic_lu()` allocation-failure status, requested-output cleanup,
    stale-output suppression, caller-owned matrix/permutation preservation,
    ...
    `sparse_analyze()`, standalone etree, postorder, colcount helpers, direct
...
partial-SVD, LU, and Cholesky comparison report families to be current. The
selected target authority is `tests/corpus/manifests/selected_report_targets.tsv`;
...
artifact `sprint190-windows-selected-comparison-cholesky`. Sprint 208 reviewed
current hosted evidence for that exact path, strengthened selected Cholesky
...
report manifest references, bounded selected Cholesky workflow markers, hosted
...
freshness. The separate `selected-comparison-freshness` workflow job is the
only Sprint 190 Windows report path, and it is limited to
`cholesky-spd-tridiag-5`. Any broader Windows report freshness promotion must
...
broad platform support, package-manager behavior, shared-library ABI,
...
- `make bench-canonical-report-freshness`
  ...
  - is mirrored by reviewed Linux and macOS hosted selected-performance
    ...
    selected row only
  - publishes only selected hosted workflow artifacts:
    `sprint168-selected-performance-freshness` on Linux and
    `sprint202-macos-selected-performance-freshness` on macOS, each with
    `retention-days: 7` and these exact paths:
...
- the reviewed hosted selected-performance lanes are Linux and macOS GitHub
  Actions oriented; their workflows set
  `SPARSE_CANONICAL_RUNNER_CONTEXT=github-actions-ubuntu-latest` on Linux,
  `SPARSE_CANONICAL_RUNNER_CONTEXT=github-actions-macos-latest` on macOS,
  `SPARSE_CANONICAL_BUILD_FLAGS=default_make_flags`, and
  `SPARSE_CANONICAL_BUILD_MODE=serial`
- the hosted workflows capture `SPARSE_CANONICAL_CPU_MODEL` from
  `/proc/cpuinfo` on Linux and `sysctl -n machdep.cpu.brand_string` on macOS
  when available, but GitHub-hosted CPU assignment can vary; the CPU model is
  context, not a stable baseline class
...
For selected QR, partial-SVD, LU, and Cholesky comparison freshness, prefer
...
reviewed Linux and macOS hosted selected-performance lanes promote only the
selected row freshness and uploaded canonical bundle metadata; they do not
promote raw timing values, timing comparisons across platforms, unselected
canonical rows, broad benchmark publication, external-library parity,
package/ABI support, broad platform proof, release proof, or state-of-the-art
performance.
