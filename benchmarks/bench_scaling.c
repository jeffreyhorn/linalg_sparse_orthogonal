...
static void run_scaling(const char *label, SparseMatrix *(*gen)(idx_t), const idx_t *sizes,
                        int nsizes, sparse_pivot_t pivot, int repeats) {
    printf("=== %s (pivot=%s, repeats=%d) ===\n", label,
           pivot == SPARSE_PIVOT_COMPLETE ? "complete" : "partial", repeats);
    printf("%-8s %-8s %-10s %-12s %-12s %-12s %-10s %-10s\n", "n", "nnz", "nnz_LU", "factor(ms)",
           "solve(ms)", "spmv(ms)", "memory", "residual");
    printf("-------- -------- ---------- ------------ ------------ "
           "------------ ---------- ----------\n");
    ...
    for (int s = 0; s < nsizes; s++) {
        ...
        printf("%-8d %-8d %-10d %-12.3f %-12.3f %-12.3f %-10zu %.3e\n", (int)n, (int)nnz_orig,
               (int)nnz_lu, t_factor / repeats, t_solve / repeats, t_spmv, sparse_memory_usage(A),
               residual);
...
int main(void) {
    ...
    run_scaling("Tridiagonal (-1, 2, -1)", make_tridiag, tri_sizes, ntri, SPARSE_PIVOT_PARTIAL, 3);

    run_scaling("Tridiagonal (-1, 2, -1)", make_tridiag, tri_sizes, ntri, SPARSE_PIVOT_COMPLETE, 3);

    /* Dense scaling (smaller sizes) */
    idx_t dense_sizes[] = {10, 20, 50, 100, 200};
    int ndense = (int)(sizeof(dense_sizes) / sizeof(dense_sizes[0]));

    run_scaling("Dense random (diag-dominant)", make_dense_wrapper, dense_sizes, ndense,
                SPARSE_PIVOT_COMPLETE, 3);
