# Sprint 127 Day 3 Compatible and Wide Residual Evidence Decision
...
trust without duplicating completed Sprint 125-126 residual lanes or implying
solution selection, minimum-norm behavior, nullspace/subspace behavior,
Q/economy behavior, sparse-mode behavior, SuiteSparse corpus behavior, or
broad QR parity.
...
on underdetermined output semantics and Q/economy/sparse-mode boundaries that
belong to later Sprint 127 and Sprint 128 owners.
...
| --- | --- | --- |
| `qr_rankdef_duplicate_5x4_compatible_zero_residual` | Deferred | Reuses the completed duplicate-column fixture family and adds only another zero-residual classification unless a future owner proves a distinct diagnostic beyond existing compatible solve, rank-only, and residual-only evidence. |
| `qr_rankdef_dependent_row_4x3_compatible_zero_residual` | Deferred | Overlaps Sprint 126's accepted incompatible dependent-row residual-only lane and existing deterministic dependent-row checks; no distinct residual-only proof value is pinned today. |
...
| SuiteSparse compatible zero-residual fixture | Deferred | Requires expected-rank metadata, threshold semantics, support tier, optional-data behavior, runtime budget, skip behavior, and diagnostics owned by Days 8-11. |
| `qr_rankdef_wide_3x5_residual_only` | Deferred | Wide residual behavior cannot be interpreted safely until rank/nullity, underdetermined output semantics, and solution-selection boundaries are explicit. |
...
| Wide sparse-mode residual-only fixture | Deferred | Requires sparse-mode output semantics, Q/economy boundaries, and residual-only proof value owned by Sprint 128 Q/economy/sparse-mode follow-through. |
...
The safer Sprint 127 action is to preserve the proof boundary and hand each
...
| Dependent-row compatible zero-residual fixture | Future QR residual owner | Show why compatible dependent-row zero residual adds a new diagnostic after Sprint 126's incompatible dependent-row residual-only lane; assert residual only. |
...
| Wide residual-only fixture | Day 4-5 nullspace/subspace owner, Day 10-13 minimum-norm owner, or Sprint 128 Q/economy owner | Define rank/nullity, underdetermined output semantics, solution-selection policy, Q/economy boundaries, residual-only proof value, diagnostics, and non-claims. |
| Wide sparse-mode residual fixture | Sprint 128 Q/economy/sparse-mode owner | Define sparse-mode output semantics, economy/Q boundaries, residual-only proof value, and validation requirements. |
