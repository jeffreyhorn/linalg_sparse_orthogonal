...
| Local selected allocation-failure proof | local-only | `make iterative-allocation-failure-gate`, `make matmul-allocation-failure-gate`, `make symbolic-allocation-failure-gate` | `test_iterative`, `test_matmul`, `test_etree`, focused registration guards, CTest `allocation_failure` labels | No broad allocation-failure coverage, OS OOM guarantee, platform parity, package/install proof, generated-tooling proof, or state-of-the-art reliability claim. |
