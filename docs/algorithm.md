...
- **`SPARSE_FM_THICK_RESTART_PERTURB=gain_noise_formal`** (Sprint 28 Days 2-3) — Sprint 27 Day 11 simplified the `gauss_noise` thick-restart variant to "random-flip with k drawn proportional to a half-Gaussian"; the formal variant adds Gaussian noise to the gain-bucket pick step (`noisy_gain = gain + sigma * |max_gain| * randn()` with linear or exponential `sigma` schedules).  Day 3's corpus sweep: catastrophic Pres_Poisson regress of +24pp (1.166× ratio) under both linear and exponential schedules; bcsstk14-class fixtures see slight wins under linear.  Ships as advisory for bcsstk14-class workloads; default stays `random_flip`.  See `docs/planning/EPIC_2/SPRINT_28/gain_noise_decision.md`.
