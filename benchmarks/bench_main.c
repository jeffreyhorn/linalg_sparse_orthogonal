...
static void benchmark(SparseMatrix *A, int repeats, sparse_pivot_t pivot, sparse_reorder_t reorder,
                      int use_cholesky) {
...
static void benchmark_tabular(const char *name, SparseMatrix *A, int repeats, sparse_pivot_t pivot,
    ...
    if (ok) {
        printf("%-20s %5d %7d %9d %6.2f  %10.3f %10.3f %10.3f %10zu  %.3e  %.3e\n", name, (int)n,
               (int)nnz_orig, (int)nnz_after,
               nnz_orig > 0 ? (double)nnz_after / (double)nnz_orig : 0.0,
               t_factor_total / repeats * 1000.0, t_solve_total / repeats * 1000.0, t_spmv,
               sparse_memory_usage(A), residual, cond_est);
    }
...
static void benchmark_spmv(SparseMatrix *A, const char *name, int iters) {
    ...
    printf("  %-20s  %dx%d  nnz=%7d  %d iters  %.6f s/iter  %.1f MFLOP/s\n", name, (int)nrows,
           (int)ncols, (int)nnz, iters, per_spmv, mflops);
...
static void benchmark_iterative(SparseMatrix *A, const char *name, int is_spd) {
    ...
    if (is_spd) {
        ...
        if (!x) {
            fprintf(stderr, "    CG: allocation failed\n");
            free(x_exact);
            free(b);
            return;
        }
        ...
        if (cerr == SPARSE_OK || cerr == SPARSE_ERR_NOT_CONVERGED)
            printf("    CG:          %4d iters  %.6f s  res=%.3e  conv=%d\n", (int)res.iterations,
                   t_cg, res.residual_norm, res.converged);
        else
    ...
    /* --- GMRES --- */
    {
        double *x = calloc((size_t)n, sizeof(double));
        if (!x) {
            fprintf(stderr, "    GMRES: allocation failed\n");
            free(x_exact);
            free(b);
            return;
        }
        ...
        if (gerr == SPARSE_OK || gerr == SPARSE_ERR_NOT_CONVERGED)
            printf("    GMRES(50):   %4d iters  %.6f s  res=%.3e  conv=%d\n", (int)res.iterations,
                   t_gm, res.residual_norm, res.converged);
        else
