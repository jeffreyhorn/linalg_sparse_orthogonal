# Epic 9 Gap-Closure Todo - 2026-06-24

This todo converts the 2026-06-24 review findings into a concrete closure
sequence. The order is deliberate: it starts by freezing the real Epic 9
product target, then attacks the structural ceilings before widening into
comparison, packaging, and final closeout work.
...
## Stage 1: Freeze the Epic 9 target and claim fence
...
**Done when:** Epic 9 starts from one fresh baseline artifact instead of only
from the Sprint 89 close notes.

### 2. Define the actual Epic 9 product target

- decide which end state the repo is now trying to earn:
  - high-end research sparse toolkit
  - production-grade sparse numerical library
  - state-of-the-art sparse linear algebra library with bounded non-claims
- write down which claims Epic 9 will and will not try to earn

**Done when:** "state of the art" means something concrete and bounded.

### 3. Freeze the new non-goal fence

- explicitly reject fake scope inflation:
  - no blanket "rewrite the library in compressed form" story
  - no fake platform symmetry
  - no fake broad complex/mixed-precision claim before real support exists
  - no fake benchmark supremacy claim without comparison evidence

**Done when:** Epic 9 has one durable anti-sprawl contract.
