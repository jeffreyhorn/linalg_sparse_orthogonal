 ...
 *
 * Demonstrates three use cases of the Sprint 20 Lanczos eigensolver:
 *
 ...
 *       and the composition with the LDL^T dispatch from Sprint 20
 *       Days 4-6.
 *   (c) Eigenvector computation with a per-pair residual check
 *       (||A*v - lambda*v|| / (|lambda| * ||v||)), confirming each
 *       returned Ritz pair satisfies the eigen-equation.
 *
 * Build:
 *   cc -O2 -Iinclude -o example_eigs examples/example_eigs.c \
 *      -Lbuild -lsparse_lu_ortho -lm
 *
 * Run (from the project root so the nos4.mtx fixture resolves):
 *   ./build/example_eigs
 */
...
int main(void) {
    printf("=== Sparse symmetric eigensolver (Sprint 20) ===\n\n");
