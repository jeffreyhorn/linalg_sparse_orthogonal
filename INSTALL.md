...
| Local selected allocation-failure proof | local-only | `make iterative-allocation-failure-gate`, `make matmul-allocation-failure-gate`, `make symbolic-allocation-failure-gate`, `make symbolic-lu-allocation-failure-gate` | `test_iterative`, `test_matmul`, `test_etree`, focused registration guards, CTest `allocation_failure` labels | Selected owners only: iterative handle workspace, `sparse_matmul()` workspaces, selected `sparse_symbolic_cholesky()` output construction, and selected `sparse_symbolic_lu()` owner. No broad allocation-failure coverage, OS OOM guarantee, platform parity, package/install proof, generated-tooling proof, or state-of-the-art reliability claim. |
