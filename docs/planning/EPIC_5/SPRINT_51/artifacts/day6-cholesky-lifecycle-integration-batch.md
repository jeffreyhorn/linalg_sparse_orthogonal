# Sprint 51 Day 6: Cholesky Lifecycle Integration Batch

## Objective

Route the shared repeated-run direct lifecycle path through the bounded
Cholesky options seam so the explicit analysis/factor path inherits the same
linked-list vs CSC backend dispatch behavior as the public one-shot Cholesky
surface, while preserving the existing one-shot contract.
...
### 1. Shared Cholesky numeric routing now uses the normal options seam

The `SPARSE_FACTOR_CHOLESKY` branch inside `sparse_factor_numeric(...)` no
longer calls `sparse_cholesky_factor(...)` directly on the already-permuted
working copy.
...
This was the real Phase-1 gap: the shared direct lifecycle path had been
correct, but it was still bypassing the public one-shot Cholesky options seam
that decides between linked-list and CSC factoring.

After the Day 6 patch:

- the explicit `sparse_analyze(...)` + `sparse_factor_numeric(...)` path
  inherits the same backend-selection behavior as the one-shot Cholesky path
- CSC writeback behavior remains governed by the existing Cholesky options
  surface instead of a separate lifecycle-only implementation branch

### 3. One-shot Cholesky APIs remain intact
...
- bounded `sparse_cholesky_factor_opts(...)` with AMD reordering
- explicit `sparse_analyze(...)` + `sparse_factor_numeric(...)` +
  `sparse_factor_solve(...)`

on a `200x200` tridiagonal SPD matrix.

That size is intentional: it sits above the default CSC auto-routing
threshold, so the test proves parity across the seam that Day 6 actually
changed.
...
- `bench_refactor_csc` continued to show strong CSC refactor wins on larger
  SuiteSparse cases:
  - `bcsstk14`: `speedup_refactor=5.63`
...
Sprint 51 Day 6 made the Cholesky repeated-run route real in code:

- the shared explicit analysis path now reuses the normal Cholesky options
  seam
- linked-list vs CSC dispatch no longer differs between the one-shot options
  path and the shared lifecycle path
- one-shot Cholesky behavior remains intact for callers
