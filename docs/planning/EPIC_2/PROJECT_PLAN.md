...
| 29 | SVD, Progress Callbacks, Eigenpair Refinement, CI & Wrap-Up — **Complete** | Sparse low-rank fix (advisory `SPARSE_SVD_LOWRANK_OUTER`; memory gate -88% rss, wall gate fail), full SVD U/V (`sparse_svd_opts_t.economy=0` branch lit up), eigenpair iterative refinement (`opts.refine`; residual ≤ 1e-13 on clustered spectrum), progress/cancel callbacks (10 routines; `SPARSE_ERR_CANCELLED`), Windows + macOS CI (Apple Clang + Homebrew GCC), coverage 95→80 calibration (Day-12 measured 81.3%), CI bench-step fix (`make bench-fast`, ~63 s vs 6 h timeout), Sprint-28 deferral absorption (`bench_reorder --reorder-via-analyze` + macOS-15+ TSan routed to existing Linux job), Epic 2 retrospective.  Zero production default flips; 4 new opt-in APIs (progress_cb fields + economy=0 + refine + SVD_LOWRANK_OUTER).  All quality gates clean. | 174 hrs (~166 actual) |
