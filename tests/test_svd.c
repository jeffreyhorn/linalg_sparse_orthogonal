...
static SparseMatrix *build_svd_external_ref_wide_fullrank_4x6(void) {
    static const double values[SVD_EXTERNAL_WIDE_FULLRANK_ROWS][SVD_EXTERNAL_WIDE_FULLRANK_COLS] = {
    ...
    SparseMatrix *A =
        sparse_create(SVD_EXTERNAL_WIDE_FULLRANK_ROWS, SVD_EXTERNAL_WIDE_FULLRANK_COLS);
    if (!A)
        return NULL;

    for (idx_t i = 0; i < SVD_EXTERNAL_WIDE_FULLRANK_ROWS; i++) {
        for (idx_t j = 0; j < SVD_EXTERNAL_WIDE_FULLRANK_COLS; j++) {
            if (values[i][j] != 0.0 && !tf_svd_insert_or_free(&A, i, j, values[i][j]))
                return NULL;
        }
    }
...
static void test_svd_external_dense_reference_wide_fullrank_4x6(void) {
#ifdef _WIN32
    SKIP_TEST("external SVD dense reference helper is not enabled on Windows");
#else
    double sigma_ref[SVD_EXTERNAL_WIDE_FULLRANK_SIGMA] = {0.0};
    char reason[256] = {0};
    int ref_status = read_svd_external_reference_singular_values("svd_wide_fullrank_4x6", sigma_ref,
                                                                 SVD_EXTERNAL_WIDE_FULLRANK_SIGMA,
                                                                 reason, sizeof(reason));
    if (ref_status == TF_EXTERNAL_REFERENCE_SKIP)
        SKIP_TEST(reason);
    if (ref_status != TF_EXTERNAL_REFERENCE_OK) {
    ...
    SparseMatrix *A = build_svd_external_ref_wide_fullrank_4x6();
    ASSERT_NOT_NULL(A);
    if (!A)
        return;
    ...
    ASSERT_EQ(svd.k, SVD_EXTERNAL_WIDE_FULLRANK_SIGMA);
    ASSERT_NOT_NULL(svd.sigma);
    if (!svd.sigma) {
        sparse_svd_free(&svd);
        sparse_free(A);
    ...
    double max_diff = 0.0;
    for (idx_t i = 0; i < SVD_EXTERNAL_WIDE_FULLRANK_SIGMA; i++) {
        double diff = fabs(svd.sigma[i] - sigma_ref[i]);
        if (diff > max_diff)
            max_diff = diff;
    }
    printf("    external SVD dense ref wide_fullrank_4x6: max |sigma-sigma_ref| = %.3e\n",
           max_diff);
    ASSERT_TRUE(max_diff < 1e-8);
...
#include "test_svd_partial_helpers.h"
...
static void test_svd_rank_full(void) { tf_svd_test_rank_full(); }

/* SVD rank: rank-deficient matrix */
static void test_svd_rank_deficient(void) { tf_svd_test_rank_deficient(); }

/* SVD rank: nearly singular (large condition number) */
static void test_svd_rank_nearly_singular(void) { tf_svd_test_rank_nearly_singular(); }
...
static void test_svd_rank_null(void) { tf_svd_test_rank_null(); }

/* Pseudoinverse: diagonal matrix */
static void test_pinv_diagonal(void) { tf_svd_test_pinv_diagonal(); }

/* Pseudoinverse: Moore-Penrose condition A * A^+ * A ≈ A */
static void test_pinv_moore_penrose(void) { tf_svd_test_pinv_moore_penrose(); }

/* Pseudoinverse: NULL inputs */
static void test_pinv_null(void) { tf_svd_test_pinv_null(); }

/* Pseudoinverse: rectangular tall matrix — verify A * A^+ * A ≈ A */
static void test_pinv_rectangular(void) { tf_svd_test_pinv_rectangular(); }
...
static void test_lowrank_diagonal(void) { tf_svd_test_lowrank_diagonal(); }

/* Low-rank: error matches theoretical bound */
static void test_lowrank_error_bound(void) { tf_svd_test_lowrank_error_bound(); }

/* Low-rank: NULL and bad args */
static void test_lowrank_errors(void) { tf_svd_test_lowrank_errors(); }
...
int main(void) {
    ...
    RUN_TEST(test_svd_external_dense_reference_wide_fullrank_4x6);
