# Sprint 64 Day 2: Validation Baseline and Touched-Surface Recheck
...
Freeze the validation and truthfulness baseline that later Sprint 64 backend,
kernel, and build-option implementation work must preserve before the sprint
moves into the deeper hotspot audit and abstraction design.
...
- targeted Sprint 64 rerun-set presence:
  - `build/`
...
This remains consistent with the repo’s current Sprint 63 close discipline and
...
than needing a wording-reconciliation batch just to start backend or
build-option implementation work.
...
- selected dense-kernel and CSC/supernodal follow-through
- representative direct example sanity checks after behavior-affecting edits
- bounded benchmark proof for the first backend-aware path
- adjacent regression verification so Sprint 64 does not widen unrelated
  solver-support or packaging claims by accident
...
- bounded `*.c` / `*.h` days:
  - run:
    ...
    - `make lint`
    - `make test`
- substantial backend, build-option, or performance-sensitive code days:
  - prefer:
    - `make quality-review-full`
...
Sprint 64 now has a written validation baseline that matches the live repo:

- strongest local reviewed baseline unchanged
- reviewed CMake parity anchor unchanged
- rerun set fixed from the current build tree around dense-kernel, CSC,
  supernodal, benchmark, and adjacent proof surfaces
