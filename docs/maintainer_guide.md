...
- Sprint 201 SVD helper boundary:
  - `tests/test_svd_helpers.h` owns the selected SVD rank, pseudoinverse, and
    dense low-rank helper implementations moved during Sprint 201
  - `tests/test_svd.c` remains the registered SVD proof-owner binary; keep
    `main` and the selected `RUN_TEST(...)` registrations there
  - keep the helper header family-local and header-only; do not add it to
    Makefile source/test lists, CMake `add_sparse_test(...)`, or
    `build-metadata/library_sources.txt`
  - run `make svd-helper-guard` after SVD helper-layout changes to check
    ...
    ownership, dependency includes, and header-only registration boundaries
